...
GO
/*
The column [dbo].[SPUR].[Status] is being dropped, data loss could occur.
*/

IF EXISTS (select top 1 1 from [dbo].[SPUR])
...
IF EXISTS (SELECT TOP 1 1 
    ...
    BEGIN
        ...
        INSERT INTO [dbo].[tmp_ms_xx_SPUR] ([SPURID], [SPURName], [JobCOde], [EffectiveStartDate], [EffectiveEndDate], [RoleLevel], [MinimumExperienceRequiredInYear], [DesiredExperienceInYear], [Industry], [Domain], [ContentItem], [AreaOfStudy], [OtherAreaOfStudy], [LicenseAndCertificate], [OtherLicnseAndCertificate], [Membership], [OtherMembership], [BatchID], [ApproverEmailID], [ActiveFlag], [SubmittedTimeStamp], [EndUserCreatedBy], [EndUserCreatedTimestamp], [EndUserModifiedBy], [EndUserModifiedTimestamp], [BackendUserModifiedBy], [BackendUserModifiedTimestamp])
        SELECT   [SPURID],
                 [SPURName],
                 [JobCOde],
                 [EffectiveStartDate],
