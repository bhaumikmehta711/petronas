...
def spur_job_profile(
    ...
    spur_df = pd.read_excel(spur_data_file_path)
    ...
    experience_df["profile_code"] = experience_df["SPUR ID"].str.replace(" ", "_") + "_" + experience_df["role_level"]
    ...
    def talent_profile(wb, spur_df, experience_df, job_dat_dir):
        ...
        talent_profile_df = pd.DataFrame(talent_profile.range("B14:K{}".format(row_end)).value)
        ...
        if len(profile_code_missing_list) != 0:
            ...
            with open(log_dir + "\\" + f"{skg_name}_job_code_missing.txt", "w") as f:
                f.write("\n".join("{}) ".format(i) + j for i, j in enumerate(profile_code_missing_list, start=1)))
        else:
            if f"{skg_name}_profile_code_missing.txt" in log_dir_list:
                os.remove(log_dir + "\\" + f"{skg_name}_profile_code_missing.txt")
        ...
        talent_profile_df.to_csv(
            job_dat_dir + "\\" + f"{skg_name}_TalentProfile.dat",
            header=None,
            index=None,
            sep="|",
            mode="w",
    ...
    def profile_relation(wb, job_dat_dir):
        ...
        profile_relation_df = pd.DataFrame(profile_relation.range("B14:K{}".format(row_end)).value)
        ...
        profile_relation_df.to_csv(
            job_dat_dir + "\\" + f"{skg_name}_ProfileRelation.dat",
            header=None,
            index=None,
            sep="|",
            mode="w",
    ...
    def model_profile_info(wb, job_dat_dir):
        ...
        ModelProfileExtraInfo_df = pd.DataFrame(ModelProfileExtraInfo.range("B14:I{}".format(row_end)).value)
        ...
        ModelProfileExtraInfo_df.to_csv(
            job_dat_dir + "\\" + f"{skg_name}_ModelProfileExtraInfo.dat",
            header=None,
            index=None,
            sep="|",
            mode="w",
    ...
    def profile_attachment(wb, job_dat_dir):
        ...
        ProfileAttachment_df = pd.DataFrame(ProfileAttachment.range("B14:K{}".format(row_end)).value)
        ...
        ProfileAttachment_df.to_csv(
            job_dat_dir + "\\" + f"{skg_name}_ProfileAttachment.dat",
            header=None,
            index=None,
            sep="|",
            mode="w",
    ...
    def profile_item_other_descriptor(wb, spur_df, job_dat_dir):
        ...
        ProfileItem_OtherDescriptor_df = pd.DataFrame(
            ProfileItem_OtherDescriptor.range("B12:R{}".format(row_end)).value
        )
        ...
        ProfileItem_OtherDescriptor_df.to_csv(
            job_dat_dir + "\\" + f"{skg_name}_ProfileItem-OtherDescriptor.dat",
            header=None,
            index=None,
            sep="|",
            mode="w",
    ...
    def profile_item_risk(wb, spur_df, job_dat_dir):
        ...
        ProfileItem_Risk_df.to_csv(
            job_dat_dir + "\\" + f"{skg_name}_ProfileItem-Risk.dat",
            header=None,
            index=None,
            sep="|",
            mode="w",
    ...
    def profile_item_exp_required(wb, experience_df, job_dat_dir):
        ...
        ProfileItem_ExperienceRequired_df.to_csv(
            job_dat_dir + "\\" + f"{skg_name}_ProfileItem-ExperienceRequired.dat",
            header=None,
            index=None,
            sep="|",
            mode="w",
    ...
    def profile_item_competency_LC(wb, LC_file_path, job_dat_dir, LC_content_item_list):
        ...
        for sheet in sheets:
            ...
            # extract numbers
            lc_columns = lc_df.columns
            for col in lc_columns[1:]:
            ...
            competency_lc = lc_df.iloc[:, [0]].values.tolist()
            min_lc = lc_df.iloc[:, [1]].values.tolist()
            max_lc = lc_df.iloc[:, [-1]].values.tolist()
            #     empty_list.extend(competency_lc)
            if sheet == "Executive":
                competency_lc_dict.update({"E": competency_lc})
                min_lc_dict.update({"E": min_lc})
                max_lc_dict.update({"E": max_lc})

            elif sheet == "Manager":
                competency_lc_dict.update({"M": competency_lc})
                min_lc_dict.update({"M": min_lc})
                max_lc_dict.update({"M": max_lc})

            elif sheet == "Senior Managers":
                competency_lc_dict.update({"SM": competency_lc})
                min_lc_dict.update({"SM": min_lc})
                max_lc_dict.update({"SM": max_lc})

            elif sheet == "Staff":
                competency_lc_dict.update({"S": competency_lc})
                min_lc_dict.update({"S": min_lc})
                max_lc_dict.update({"S": max_lc})

            elif sheet == "Principal":
                competency_lc_dict.update({"P": competency_lc})
                min_lc_dict.update({"P": min_lc})
                max_lc_dict.update({"P": max_lc})

            elif sheet == "Custodian":
                competency_lc_dict.update({"C": competency_lc})
                min_lc_dict.update({"C": min_lc})
                max_lc_dict.update({"C": max_lc})

            elif sheet == "General Managers":
                competency_lc_dict.update({"GM": competency_lc})
                min_lc_dict.update({"GM": min_lc})
                max_lc_dict.update({"GM": max_lc})

            else:
                competency_lc_dict.update({"SGM": competency_lc})
                min_lc_dict.update({"SGM": min_lc})
                max_lc_dict.update({"SGM": max_lc})
        ...
        for ur in [
            ...
            for row in range(14, row_end_talent_profile + 1)
            if wb.sheets[1].range((row, 4)).value != None
        ]:
            if re.search("_Chief$", ur):
                #         print(ur)
                lc_list.extend(competency_lc_dict["SGM"])
                min_list.extend(min_lc_dict["SGM"])
                max_list.extend(max_lc_dict["SGM"])

            elif re.search("_CUSTODIAN$", ur):
                lc_list.extend(competency_lc_dict["C"])
                min_list.extend(min_lc_dict["C"])
                max_list.extend(max_lc_dict["C"])

            elif re.search("_PRINCIPAL$", ur):
                lc_list.extend(competency_lc_dict["P"])
                min_list.extend(min_lc_dict["P"])
                max_list.extend(max_lc_dict["P"])

            elif re.search("_STAFF$", ur):
                lc_list.extend(competency_lc_dict["S"])
                min_list.extend(min_lc_dict["S"])
                max_list.extend(max_lc_dict["S"])

            elif re.search("_GM$", ur):
                lc_list.extend(competency_lc_dict["GM"])
                min_list.extend(min_lc_dict["GM"])
                max_list.extend(max_lc_dict["GM"])

            elif re.search("_SM$", ur):
                lc_list.extend(competency_lc_dict["SM"])
                min_list.extend(min_lc_dict["SM"])
                max_list.extend(max_lc_dict["SM"])

            elif re.search("_MANAGER$", ur):
                lc_list.extend(competency_lc_dict["M"])
                min_list.extend(min_lc_dict["M"])
                max_list.extend(max_lc_dict["M"])

            else:
                lc_list.extend(competency_lc_dict["E"])
                min_list.extend(min_lc_dict["E"])
                max_list.extend(max_lc_dict["E"])
        ...
        source_system_id = [
            [wb.sheets[0].range((row, 11)).get_address(True, False, True)] * 8
            for row in range(14, row_end_talent_profile + 1)
            if wb.sheets[0].range((row, 11)).value != None
        ]
        ...
        ProfileItem_Competency_LC_df.to_csv(
            job_dat_dir + "\\" + f"{skg_name}_ProfileItem-Competency_LC.dat",
            header=None,
            index=None,
            sep="|",
            mode="w",
    ...
    def profile_item_competency_TC(wb, TC_file_path, job_dat_dir, TC_content_item_list):
        ...
        technical_competency_df = technical_competency_df.replace(to_replace=0, value=np.nan)
        ...
        technical_competency_df.dropna(subset=[oracle_column], inplace=True)
        ...
        min_tc_list = []
        max_tc_list = []
        job_tc_important_list = []

        base_columns = ["SPUR ID", "TI Name", "TI Number", oracle_column]
        row_end_talent_profile = wb.sheets[1].range("D" + str(wb.sheets[1].cells.last_cell.row)).end("up").row
        for row in range(14, row_end_talent_profile + 1):
            if wb.sheets[0].range((row, "K")).value == None:
                break

            ur_id = re.sub("_.+", "", wb.sheets[0].range((row, "K")).value)
            role_level = re.sub(".+_", "", wb.sheets[0].range((row, "D")).value)
            # print(ur_id)

            if ur_id in spur_id_tc_occ.index:
                temp_df = (
                    technical_competency_df[technical_competency_df["SPUR ID"] == ur_id]
                    .replace("N/A", np.nan, regex=True)
                    .dropna(axis=1, how="all")
                )
                tc_ref_list.extend([wb.sheets[0].range((row, "K")).get_address(True, False, True)] * len(temp_df))
            ...
            if ur_id in technical_competency_df["SPUR ID"].values.tolist():
                # tc_name_list.extend(technical_competency_df[technical_competency_df['SPUR ID'] == ur_id][[oracle_column]].values.tolist())
                job_tc_important_list.extend(
                         technical_competency_df[technical_competency_df["SPUR ID"] == ur_id][["Important"]].values.tolist()
                     )
                # temp_df = technical_competency_df[technical_competency_df['SPUR ID'] == ur_id].dropna(axis=1, how='all')
                tc_name_list.extend(temp_df[[oracle_column]].values.tolist())
                # ur_id_columns = list(set(temp_df) - set(base_columns))
                # temp_df = temp_df[ur_id_columns]
                # rating_cols = [
                #     "A1",
                #     "A2",
                #     "A3",
                ...
                if role_level == "EXECUTIVE":
                    rating_cols = ["A1", "A2", "A3", "D1"]
                elif role_level == "MANAGER":
                    rating_cols = ["D2", "D3"]
                elif role_level == "SM":
                    rating_cols = ["M1", "M2"]
                elif role_level == "STAFF":
                    rating_cols = ["Staff", "D2"]
                elif role_level == "PRINCIPAL":
                    rating_cols = ["Principal", "M1"]
                elif role_level == "CUSTODIAN":
                    rating_cols = ["Custodian", "C1"]
                elif role_level == "GM":
                    rating_cols = ["C1", "C2"]
                else:
                    rating_cols = ["H1", "H2"]
                temp_df = temp_df[list(set(temp_df.columns) & set(rating_cols))]
                temp_df = temp_df.applymap(
                    lambda x: int(float(re.sub("\s+|\u200b", "", str(x)))) if str(x) != "nan" else x
                )
                if ur_id == "F02-026":
                    print(temp_df)
                    print(temp_df.min(axis=1).tolist())
                    print(temp_df.max(axis=1).tolist())
                    # print(temp_df.select_dtypes(include="number").columns)

                # numeric_columns = temp_df.select_dtypes(include="number").columns

                min_tc_list.extend([[x] for x in temp_df.min(axis=1).tolist()])
                max_tc_list.extend([[x] for x in temp_df.max(axis=1).tolist()])
                # min_tc_list.extend([[x] for x in temp_df.min(axis=1).tolist() if not np.isnan(x)])
                # max_tc_list.extend([[x] for x in temp_df.max(axis=1).tolist() if not np.isnan(x)])
        ...
        # column J
        ProfileItem_Competency_TC.range((13, "J")).value = job_tc_important_list

        last_row_tc = 13 + len(tc_ref_list) - 1
        # Column BCDFGIKMO
        ...
        ProfileItem_Competency_TC_df = pd.DataFrame(
            ProfileItem_Competency_TC.range("B13:P{}".format(last_row_tc)).value
        ).dropna(subset=[10])
        ...
        ProfileItem_Competency_TC_df.to_csv(
            job_dat_dir + "\\" + f"{skg_name}_ProfileItem-Competency_TC.dat",
            header=None,
            index=None,
            sep="|",
            mode="w",
    ...
    def profile_item_degree(wb, degree_df, job_dat_dir, AreaOfStudy_content_item_list):
        ...
        ProfileItem_Degree_df = pd.DataFrame(ProfileItem_Degree.range("B12:Q{}".format(row_end_degree_sheet)).value)
        ...
        ProfileItem_Degree_df.to_csv(
            job_dat_dir + "\\" + f"{skg_name}_ProfileItem-Degree.dat",
            header=None,
            index=None,
            sep="|",
            mode="w",
    ...
    def profile_item_language(wb, job_dat_dir):
        ...
        ProfileItem_Language_df.to_csv(
            job_dat_dir + "\\" + f"{skg_name}_ProfileItem-Language.dat",
            header=None,
            index=None,
            sep="|",
            mode="w",
    ...
    def profile_item_membership(wb, membership_df, job_dat_dir, membership_content_item_list):
        ...
        if membership_list != []:
            ...
            ProfileItem_Membership_df = pd.DataFrame(
                ProfileItem_Membership.range("B12:M{}".format(row_end_membership_sheet)).value
            )
            ...
            ProfileItem_Membership_df.to_csv(
                job_dat_dir + "\\" + f"{skg_name}_ProfileItem-Membership.dat",
                header=None,
                index=None,
                sep="|",
                mode="w",
    ...
    def profile_item_awards(wb, awards_df, job_dat_dir, awards_content_item_list):
        ...
        if awards_list != []:
            ...
            ProfileItem_Awards_df = pd.DataFrame(
                ProfileItem_Awards.range("B12:N{}".format(row_end_awards_sheet)).value
            )
            ...
            ProfileItem_Awards_df.to_csv(
                job_dat_dir + "\\" + f"{skg_name}_ProfileItem-Awards.dat",
                header=None,
                index=None,
                sep="|",
                mode="w",
    ...
    def profile_item_license(wb, license_df, job_dat_dir, license_content_item_list):
        ...
        if license_list != []:
            ...
            ProfileItem_License_df.to_csv(
                job_dat_dir + "\\" + f"{skg_name}_ProfileItem-License.dat",
                header=None,
                index=None,
                sep="|",
                mode="w",
